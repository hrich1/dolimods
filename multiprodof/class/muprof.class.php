 ...
 * Class for muprof
 */
class MuPrOf extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'multiprodof_muprof';
	...
	public $fields = array(
		...
		'fk_soc' => array('type'=>'integer:Societe:societe/class/societe.class.php:1', 'label'=>'ThirdParty', 'picto'=>'company', 'enabled'=>1, 'visible'=>-1, 'position'=>20, 'notnull'=>-1, 'index'=>1, 'css'=>'maxwidth400'),
		'fk_warehouse' => array('type'=>'integer:Entrepot:product/stock/class/entrepot.class.php:0', 'label'=>'WarehouseForProduction', 'picto'=>'stock', 'enabled'=>1, 'visible'=>1, 'position'=>30),
		
		'date_start_planned' => array('type'=>'datetime', 'label'=>'DateStartPlannedMo', 'enabled'=>1, 'visible'=>1, 'position'=>31, 'notnull'=>-1, 'index'=>1, 'help'=>'KeepEmptyForAsap'),
		'date_end_planned' => array('type'=>'datetime', 'label'=>'DateEndPlannedMo', 'enabled'=>1, 'visible'=>1, 'position'=>32, 'notnull'=>-1, 'index'=>1,),
		'numermolde' => array('type'=>'varchar(25)', 'label'=>'numermolde', 'enabled'=>1, 'visible'=>1, 'position'=>33),
		
		'temper' => array('type'=>'varchar(50)', 'label'=>'Temper', 'enabled'=>1, 'visible'=>0, 'position'=>45),
		'water_circulates' => array('type'=>'varchar(50)', 'label'=>'Water_circulates', 'enabled'=>1, 'visible'=>0, 'position'=>50),
		'rack_in_temp' => array('type'=>'varchar(50)', 'label'=>'Rack_in_temperature', 'enabled'=>1, 'visible'=>0, 'position'=>55),
		'ejector_safety' => array('type'=>'varchar(50)', 'label'=>'Ejector_safety', 'enabled'=>1, 'visible'=>0, 'position'=>60),
		'mold_cleaning' => array('type'=>'varchar(50)', 'label'=>'Mold_cleaning', 'enabled'=>1, 'visible'=>0, 'position'=>65),
		'dater' => array('type'=>'varchar(50)', 'label'=>'Dater', 'enabled'=>1, 'visible'=>0, 'position'=>70),
		'signature' => array('type'=>'datetime', 'label'=>'Signature', 'enabled'=>1, 'visible'=>0, 'position'=>500, 'notnull'=>0,),
		'fk_user_quality' => array('type'=>'integer', 'label'=>'Operator', 'enabled'=>1, 'visible'=>0, 'position'=>510, 'notnull'=>0, 'foreignkey'=>'user.rowid',),
		'notes' => array('type'=>'html', 'label'=>'Notes', 'enabled'=>1, 'visible'=>0, 'position'=>520, 'notnull'=>-1,),
		
		'fk_project' => array('type'=>'integer:Project:projet/class/project.class.php:1:fk_statut=1', 'label'=>'Project', 'picto'=>'project', 'enabled'=>1, 'visible'=>-1, 'position'=>80, 'notnull'=>-1, 'index'=>1, 'css'=>'minwidth200 maxwidth400'),
		'note_public' => array('type'=>'html', 'label'=>'NotePublic', 'enabled'=>1, 'visible'=>0, 'position'=>82, 'notnull'=>-1,),
		'note_private' => array('type'=>'html', 'label'=>'NotePrivate', 'enabled'=>1, 'visible'=>0, 'position'=>85, 'notnull'=>-1,),
		'date_creation' => array('type'=>'datetime', 'label'=>'DateCreation', 'enabled'=>1, 'visible'=>0, 'position'=>500, 'notnull'=>1,),
		'tms' => array('type'=>'timestamp', 'label'=>'DateModification', 'enabled'=>1, 'visible'=>-2, 'position'=>501, 'notnull'=>1,),
		'date_valid' => array('type'=>'datetime', 'label'=>'DateValidation', 'enabled'=>1, 'visible'=>-2, 'position'=>502,),
		'fk_user_creat' => array('type'=>'integer', 'label'=>'UserAuthor', 'enabled'=>1, 'visible'=>0, 'position'=>510, 'notnull'=>1, 'foreignkey'=>'user.rowid',),
	...
	public $entity;
	public $temper;
	public $water_circulates;
	public $rack_in_temp;
	public $ejector_safety;
	public $mold_cleaning;
	public $dater;
	public $notes;
	public $signature;
	public $fk_user_quality;
	public $fk_warehouse;
	public $fk_soc;
	 ...
	 */
	public $table_element_line = 'multiprodof_production';

	/**
	 * @var string    Field with ID of parent key if this field has a parent
	 */
	public $fk_element = 'fk_muprof';

	/**
	 * @var string    Name of subtable class that manage subtable lines
	 */
	public $class_element_line = 'MuPrOfLine';
	 ...
	 */
	protected $childtablesoncascade = array('multiprodof_production');

	/**
	 * @var MuPrOfLine[]     Array of subtable lines
	 */
	public $lines = array();
	...
	public function fetchLinesLinked($role, $lineid = 0, $lineid_row = 0)
	{
		$resarray = array();
		$mostatic = new MuPrOfLine($this->db);
		...
		if($lineid_row==0)
		{
			if ($lineid > 0) $sql .= ' AND t.fk_multiprodof_production = '.$lineid;
			else $sql .= 'AND t.fk_muprof = '.$this->id;
		}
		else
			$sql .= 'AND t.rowid = '.$lineid_row;
	...
	public function countMovements()
	{
		...
		$sql .= " WHERE sm.origintype = 'muprof' and sm.fk_origin = ".$this->id;
	...
	public function updateProduction(User $user, $notrigger = true)
	{
		...
		if (!$error)
		{
			...
			$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'mrp_production WHERE fk_muprof = '.$this->id;
			$this->db->query($sql);

			$moline = new MuPrOfLine($this->db);

			// Line to produce
			$moline->fk_muprof = $this->id;
			$moline->qty = $this->qty;
	...
	public function getNextNumRef($prod)
	{
		global $langs, $conf;
		$langs->load("muprof");
		/*echo $conf->global->MULTIPRODOF_MUPROF_ADDON.".php";
		exit;*/
		if (!empty($conf->global->MULTIPRODOF_MUPROF_ADDON))
		{
			$mybool = false;

			$file = $conf->global->MULTIPRODOF_MUPROF_ADDON.".php";
			$classname = $conf->global->MULTIPRODOF_MUPROF_ADDON;
			...
			{
				$dir = dol_buildpath($reldir."multiprodof/core/modules/");
			...
			print $langs->trans("Error")." ".$langs->trans("Error_MULTIPRODOF_MUPROF_ADDON_NotDefined");
			return "";
		}
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/multiprodof/muprof_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new MuPrOfLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_muprof = '.$this->id));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/multiprodof/doc/";
	...
	public function printOriginLine($line, $var, $restrictlist = '', $defaulttpldir = '/core/tpl', $selectedLines = array())
	{
		...
		if (!empty($line->fk_product))
		{
			...
			// If origin MULTIPRODOF line is not a product, but another MRP
			// TODO
		}
		...
		$tpl = DOL_DOCUMENT_ROOT.'/multiprodof/tpl/originproductline.tpl.php';
		$res = include $tpl;
	}
	...
	{
		$tables = array('multiprodof_muprof');

		return CommonObject::commonReplaceThirdparty($db, $origin_id, $dest_id, $tables);
	}
	/**
	 * Function used to update the fk_father field in multiprodof_production, when the OF was cloned and the toconsume lines were not linked to their parent, it is only done when the toproduce lines have the row_id data different from fk_refline
	 *
	 * @param DoliDB 	$db 			Database handler
	 * @param  User $user      User that modifiesint
	 */
	public function verify_clone_lines($db, $user)
	{
		$c = 0;
		foreach ($this->lines as $line) {
			
			if ($line->role == 'toproduce') {
				if($line->fk_refline != $line->id)
				{
					//echo $line->fk_refline."Diferentes".$line->id;
					$objlines = new MuPrOfLine($db);
					$objlines->db->begin();
					
					$objlines->fetch($line->id);
					$objlines->fk_refline = $line->id;
					$res = $objlines->update($user, false);
				
					if ($res <= 0)
					{
						setEventMessages($this->error, $this->errors, 'errors');
						$objlines->db->rollback();	
					}
					else
					{
						$objlines->db->commit();
						//Modify childs toconsume of this line toproduce
						foreach ($this->lines as $linec) {
							if ($linec->role == 'toconsume' && $linec->fk_father == $line->fk_refline) {
								$objlinec = new MuPrOfLine($db);
								$objlinec->db->begin();
								
								$objlinec->fetch($linec->id);
								$objlinec->fk_father = $line->id;
								$res = $objlinec->update($user, false);
							
								if ($res <= 0)
								{
									setEventMessages($this->error, $this->errors, 'errors');
									$objlinec->db->rollback();	
								}
								else
								{
									$objlinec->db->commit();
		...
		if($c>0)
		{
			//Start Delete lines consumed or produced from clone OF
			$this->db->begin();
			$sql = 'DELETE';
	...
	public function CreateOrder($user, $selec_produt, $selec_produtall, $fororder, $notrigger = 0)
	{
		global $langs, $conf;
		
		/*print_r($selec_produt);
		print_r($selec_produtall);
		print_r($fororder);
		exit;*/
		...
		$com->ref            = '';
		$com->socid          = $this->fk_soc;	// Put id of third party (rowid in llx_societe table)
		$com->date           = dol_now();
		$com->note_public    = '';
		$com->note_private   = '';
		$com->source         = $this->id;
		$com->remise_percent = 0;
		//$com->model_pdf = 'albaran';
		$com->array_options["numermolde"] = $this->numermolde;
		$com->array_options["fk_muprof"] = $this->id;
		$com->array_options["fk_muprof_ref"] = $this->ref;
		

		$linesord = 0;
		foreach($this->lines as $lineof)
		{
			
			if($lineof->role=="toproduce" && in_array($lineof->id, $selec_produt) )
			{
				$index = array_search($lineof->id, $selec_produtall);
				
				//echo "===>".$lineof->id."===>".$index."<br>";
				
				//Verify if the line toproduce produced
				$arraytoproduced = $this->fetchLinesLinked('toproduce', 0, $lineof->id);
				$nbtoProduced = 0;
				foreach ($arraytoproduced as $lineproduced) {
					$nbtoProduced += $lineproduced['qty'];
				}
					
				/*$arrayproduced = $this->fetchLinesLinked('produced', $lineof->id);
				$nbProduced = 0;
				...
				if($nbtoProduced==$nbProduced && $nbtoProduced>0)
				{*/
					$product = new Product($this->db);
					$product->fetch($lineof->fk_product);
					$orderline1 = new OrderLine($this->db);
					$orderline1->fk_product=$product->id;
					$orderline1->tva_tx=$product->tva_tx;
					$orderline1->remise_percent=0;
					$orderline1->qty=$fororder[$index];
					$orderline1->subprice = $product->price;
					$orderline1->array_options["fk_muprof_line"] = $lineof->id;
		...
		if($linesord>0)
		{
			...
			if ($idobject > 0)
			{
				...
				$error++;
				dol_print_error($this->db, $com->error);
			}
		...
		if(!$error)
		{
			...
			$coma = new Commande($this->db);
			$coma->fetch($idobject);
			$coma->generateDocument($coma->model_pdf, $outputlangs);
		}
		...
		if (!$error)
		{
			$this->add_object_linked('order', $idobject);
		}
		// -------------------- START OBJECT LINKED --------------------
		...
		if (!$error)
		{
			...
			$this->db->commit();
			$this->db->close();
			header("Location: ../commande/card.php?id=".$idobject);
			exit;
		} else {
			$this->db->rollback();
			if($linesord==0)
			{
				setEventMessages($langs->trans("ErrorCreateOrderNoLines"), $this->errors, 'errors');
			}
			else {
				setEventMessages($langs->trans("ErrorCreateOrder"), $this->errors, 'errors');
			}
	 ...
	 *	@return	int						<0 if KO, 0=Nothing done, >0 if OK
	 */
	public function CountLinesInOrder($lines, $order = '')
	{
		$resp = array();
		foreach($lines as $line)
		{
			$sql = "select Sum(ifnull(comd.qty,0)) as Units
			from ".MAIN_DB_PREFIX."commandedet comd 
			join ".MAIN_DB_PREFIX."commandedet_extrafields comdx on comd.rowid = comdx.fk_object 
			join ".MAIN_DB_PREFIX."commande com on comd.fk_commande = com.rowid 
			join ".MAIN_DB_PREFIX."commande_extrafields comx on com.rowid = comx.fk_object
			where comx.fk_muprof = '".$this->id."'
			AND com.fk_statut<> '-1'
			AND comdx.fk_muprof_line = '".$line."' ";
			if($order!="")
				$sql .= " AND com.rowid <> '".$order."'";
			//echo $sql;
			$resql = $this->db->query($sql);
			if ($resql)
			{
				...
				if($obj->Units == "")
				{
					if($order!="")
					{
						$objlinec = new MuPrOfLine($this->db);
						$objlinec->fetch($line);
						$resp[$line] = array($objlinec->qty, 0);
					}
					else
						$resp[$line] = 0;
				}
				else
				{
					if($order!="")
					{
						$objlinec = new MuPrOfLine($this->db);
						$objlinec->fetch($line);
						$resp[$line] = array($objlinec->qty, $obj->Units);
					}
					else
						$resp[$line] = $obj->Units;
				}
...
/**
 * Class MuPrOfLine. You can also remove this and generate a CRUD class for lines objects.
 */
class MuPrOfLine  extends CommonObjectLine
{
	/**
	 * @var string ID to identify managed object
	 */
	public $element = 'multiprodof_production';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'multiprodof_production';
	...
	public $fields = array(
		...
		'fk_multiprodof_production' =>array('type'=>'integer', 'label'=>'Fk mrp production', 'enabled'=>1, 'visible'=>-1, 'position'=>150),
		'fk_stock_movement' =>array('type'=>'integer', 'label'=>'StockMovement', 'enabled'=>1, 'visible'=>-1, 'position'=>155),
		'fk_father' =>array('type'=>'integer', 'label'=>'Father_toproduce', 'enabled'=>1, 'visible'=>-1, 'position'=>185),
		'fk_refline' =>array('type'=>'integer', 'label'=>'Father_toproduce', 'enabled'=>1, 'visible'=>-1, 'position'=>190),
	...
	public $batch;
	public $role;
	public $fk_multiprodof_production;
	...
	public function setproductionslines(User $user, $notrigger = true)
	{
		global $object;
		$error = 0;
		...
		{
			// Line to produce
			$this->fk_muprof = $object->id;
			$this->qty = GETPOST('qtytoadd', 'int'); ;
			$this->fk_product = GETPOST('productidtoadd', 'int');
			$this->fk_bom = GETPOST('fk_bom', 'int');
			$this->role = 'toproduce';
			$this->position = 1;
	
			$resultlb = $this->create($user, false); // Never use triggers here
			...
			//Start set ref line for future clone proccess
			$muup = new MuPrOfLine($this->db);
			$muup->fetch($resultlb);
			$muup->fk_refline = $resultlb;
			$resupd = $muup->update($user, false); // Never use triggers here
			//End set ref line for future clone proccess
			...
			//set ref line for future clone proccess
	...
	public function updateCloneLines(User $user, $notrigger = true)
	{
		global $object;
		$error = 0;
