...
/**
 *   	\file      	muprof_card.php
 *		\ingroup    multiprodof
 *		\brief      Page to create/edit/view muprof
 */
...
require_once DOL_DOCUMENT_ROOT.'/multiprodof/lib/multiprodof_muprof.lib.php';
require_once DOL_DOCUMENT_ROOT.'/bom/class/bom.class.php';

// Load translation files required by the page
$langs->loadLangs(array("multiprodof", "other"));
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'muprofcard'; // To manage different context of search
...
$hookmanager->initHooks(array('muprofcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$object->verify_clone_lines($db,$user);
...
$permissiontodelete = $user->rights->multiprodof->writedelete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
...
if (empty($reshook))
{
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			...
			else $backtopage = DOL_URL_ROOT.'/multiprodof/muprof_card.php?id='.($id > 0 ? $id : '__ID__');
		}
	}
	...
	$triggermodname = 'NULTIPORDOF_MUPROF_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	{
		$resor = $object->CreateOrder($user,false);
	}

	// Action close produced
	if ($action == 'confirm_produced' && $confirm == 'yes' && $permissiontoadd)
	{
		$result = $object->setStatut($object::STATUS_PRODUCED, 0, '', 'MULTIPRODOF_MUPROF_PRODUCED');
		if ($result >= 0)
		{
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("MuPrOf")), '', 'multiprodof');
...
{
	print load_fiche_titre($langs->trans("ManufacturingOrder"), '', 'multiprodof');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if (!empty($object->table_element_line))
	{
		...
		//{
			...
			print '<td colspan="2" align="center" class="liste_titre">'.$langs->trans("Summary").'</td>';
			print '</tr>';
			...
			if (!empty($object->lines))
			{
				...
				foreach ($object->lines as $linep) {
					if ($linep->role == 'toproduce') {
						
						$tmpproduct = new Product($db);
						$tmpproduct->fetch($linep->fk_product);
						...
						if (!empty($object->lines))
						{
							...
							{
								if ($line->role == 'toconsume' && $line->fk_father==$linep->id)
								{
									if ($f) print ', ';
									$tmpproduct = new Product($db);
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook))
		{
			...
			if ($permissiontoadd)
			{
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&socid='.$object->fk_soc.'&action=clone&object=muprof">'.$langs->trans("ToClone").'</a>';
			}
			...
			if ($permissiontoadd)
			{
				...
				/*if ($object->status == $object::STATUS_PRODUCED)
				{
					...
					{
						print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=confirm_neworder&confirm=yes">'.$langs->trans("CreateOrder").'</a>'."\n";
					}
					else {
	...
	if ($action != 'presend')
	{
		...
		$filedir = $conf->multiprodof->dir_output.'/'.$objref;
		$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
		$genallowed = $user->rights->multiprodof->read; // If you can read, you can build the PDF to read content
		$delallowed = $user->rights->multiprodof->create; // If you can create/edit, you can remove a file on card
		print $formfile->showdocuments('multiprodof:muprof', $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $mysoc->default_lang);

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('muprof'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/multiprodof/muprof_agenda.php', 1).'?id='.$object->id.'">';
		...
		$somethingshown = $formactions->showactions($object, 'muprof', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
	...
	$diroutput = $conf->multiprodof->dir_output;
	$trackid = 'muprof'.$object->id;
