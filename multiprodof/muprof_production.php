...
if (empty($reshook))
{
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		...
		else $backtopage = DOL_URL_ROOT.'/multiprodof/muprof_production.php?id='.($id > 0 ? $id : '__ID__');
	}
	...
	if ($action == 'confirm_addconsumeline' && GETPOST('addconsumelinebutton')) {
		...
		$moline->position = 0;
		$moline->fk_father = GETPOST('BomSelect','int');
		...
		$action = '';
		unset($_GET['BomSelect']);
		unset($_POST['BomSelect']);
				...
				while (GETPOSTISSET('qty-'.$line->id.'-'.$i)) {
					...
					if ($qtytoprocess != 0) {
						...
						if (!$error) {
							...
							$moline->fk_multiprodof_production = $line->id;
							$moline->fk_stock_muprofvement = $idstockmove;
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'delete')
	{
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteMuPrOf'), $langs->trans('ConfirmDeleteMo'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action == 'validate')
	{
		...
		$text = $langs->trans('ConfirmValidateMuPrOf', $numref);
		...
		$formquestion = array();
		if (!empty($conf->multiprodof->enabled))
		{
	...
	/*
	// Ref bis
	$morehtmlref.=$form->editfieldkey("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->multiprodof->muprof->creer, 'string', '', 0, 1);
	$morehtmlref.=$form->editfieldval("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->multiprodof->muprof->creer, 'string', '', null, null, '', 1);*/
	// Thirdparty
	...
	//$object->fk_user_creat = $userstatic->getNomUrl(1, '', 0, 0, 24, 0, 'login');
	...
	if (!in_array($action, array('consumeorproduce', 'consumeandproduceall')))
	{
		...
		if (empty($reshook)) {
			...
			// Consume or produce
			if ($object->status == MuPrOf::STATUS_VALIDATED || $object->status == MuPrOf::STATUS_INPROGRESS) {
				if ($permissiontoproduce) {
					print '<a class="butAction" href="javascript:actconsumeorproduce()">'.$langs->trans('ConsumeOrProduce').'</a>';
					print '<script>function actconsumeorproduce(){ var selec_produt = $(\'input[name="selec_produt"]:checked\').val();';
					print 'if(selec_produt){window.location.href ="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=consumeorproduce&BomSelect="+selec_produt;}else{ alert("'.$langs->trans("PlisSelectProdutToProduce").'") }}</script>';
				} else {
			...
			} elseif ($object->status == MuPrOf::STATUS_DRAFT) {
			...
			if ($object->status == MuPrOf::STATUS_VALIDATED || $object->status == MuPrOf::STATUS_INPROGRESS) {
				if ($permissiontoproduce) {
					print '<a class="butAction" href="javascript:actconsumeandproduceall()">'.$langs->trans('ConsumeAndProduceAll').'</a>';
					print '<script>function actconsumeandproduceall(){ var selec_produt = $(\'input[name="selec_produt"]:checked\').val();';
					print 'if(selec_produt){window.location.href ="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=consumeandproduceall&BomSelect="+selec_produt;}else{ alert("'.$langs->trans("PlisSelectProdutToProduce").'") }}</script>';
				} else {
	...
	if (in_array($action, array('consumeorproduce', 'consumeandproduceall', 'addconsumeline','addproductionline')))
	{
	...
	if (!empty($object->table_element_line))
	{
		// Show object lines
		$object->fetchLines();
		$statsa_ = array('validate', 'confirm_validate','confirm_reopen','confirm_produced','confirm_close');
		if((!GETPOST('BomSelect','int')) && ($action == '' || in_array($action, $statsa_)))// shows a view of all the products to be produced and consumed for the user's convenience 
		{
			include(DOL_DOCUMENT_ROOT.'/multiprodof/muprof_production_tpl.php');
		}
		else
		{
		print '<div class="fichecenter">';
		...
		{
			$whithLeft = 'style="width:100% !important"';
			$ticol = 'Bom';
		}
		
		print '<div '.$whithLeft.' class="fichehalfleft">';
		print '<div class="clearboth"></div>';
		...
		//Add produccion lines
		if ($action == 'addproductionline') {
			$productbom = '';
			$qtybom = 1;
			$fk_bom = GETPOST('fk_bom');
			if (GETPOST('fk_bom', 'int'))
			{
				...
				if ($action != 'add') {
					...
					$productbom = $objectbom->fk_product;
					$qtybom = $objectbom->qty;
				}
			}
			...
			print '<tr class="liste_titre">';
			print '<td>';
			//print_r($moline->fields['fk_bom']);
			print $moline->showInputField('', 'fk_bom', $fk_bom, '', '', '', 0);
			print '</td>';
			print '<td>';
			print $form->select_produits($productbom, 'productidtoadd', '', 0, 0, -1, 2, $productbom, 0, array(), 0, '1', 0, 'maxwidth300');
			print '</td>';
			print '<td class="right"><input type="text" name="qtytoadd" value="'.$qtybom.'" class="width50 right"></td>';
			print '<td class="right"></td>';
			print '<td>';
			print '<input type="submit" class="button buttongen" name="addproductionlinebutton" value="'.$langs->trans("Add").'">';
			print '</td>';
			if ($conf->productbatch->enabled) {
				print '<td></td>';
			}
		...
		if (!empty($object->lines))
		{
			...
			foreach ($object->lines as $line) {
				if($action=="consumeorproduce" || $action=="consumeandproduceall" || GETPOST('BomSelect','int'))
				{
		...
		$hidRight = '';
		if ($action == 'addproductionline' || !GETPOST('BomSelect','int'))
		{
			$hidRight = 'style="display:none;"';
		}
		
		print '<div '.$hidRight.' class="fichehalfright">';
		print '<div class="clearboth"></div>';

		$newlinetext = '';
		if ($action != 'consumeorproduce' && $action != 'consumeandproduceall') {
			$newlinetext = '<a href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=addconsumeline&BomSelect='.GETPOST('BomSelect','int').'">'.$langs->trans("AddNewConsumeLines").'</a>';
		}
		...
		if ($action == 'addconsumeline') {
			...
			print '<td class="right"></td>';
			print '<td>';
			print '<input type="hidden" name="BomSelect" value="'.GETPOST('BomSelect','int').'">';
		...
		if (!empty($object->lines))
		{
			...
			foreach ($object->lines as $line) {
				if ($line->role == 'toconsume' && $line->fk_father==GETPOST('BomSelect','int')) {
     	...
     	$(document).ready(function () {
			jQuery('#fk_bom').change(function() {
				...
				if (jQuery('#fk_bom').val() > 0)
				{
					...
					window.location.href = '<?php echo $_SERVER["PHP_SELF"] ?>?id=<?php echo $id?>&action=addproductionline&fk_bom='+jQuery('#fk_bom').val();
...
<!--<script>
$(document).ready(function () {
	jQuery('.selec_produt').change(function(e) {
 	        	//console.log(this);
 	        	if(this)
 	        	{
 	        		window.location.href = '<?php echo $_SERVER["PHP_SELF"] ?>?id=<?php echo $id; ?>&action=viewconsume&BomSelect='+this.value;
 	        	}
			 });
