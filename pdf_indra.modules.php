...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_indra.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from indra model
 */
...
/**
 *	Class to generate the customer invoice PDF with template Indra
 */
class pdf_indra extends ModelePDFFactures
{
	 /**
...
$conf->global->MAIN_PDF_DATA_ON_FIRST_PAGE=1;
		// Translations
		$langs->loadLangs(array("main", "bills"));

		$this->db = $db;
		$this->name = "indra";
		$this->description = $langs->trans('PDFIndraDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					$StrPed = explode('Pedido ',$object->lines[$i]->desc);
					$Ped = explode(' - ',$StrPed[1]);
					$object->lines[$i]->Pedidof_ = $Ped[0];
				}
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					for ($j = 0; $j < $nblines; $j++)
					{
						if($object->lines[$i]->Pedidof_ < $object->lines[$j]->Pedidof_ )
						{
							
							$back = $object->lines[$i];
							$object->lines[$i] = $object->lines[$j];
							$object->lines[$j] = $back;
						}
					}
				}
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i == ($nblines - 1))
						{
							...
							$pdf->line($this->marge_gauche, $posy__, $this->page_largeur - $this->marge_droite, $posy__);
					...
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxup, $curY);
					$pdf->MultiCell($this->posxqty - $this->posxup - 0.8, 4, $qty, 0, 'R'); // Enough for 6 chars
					
					// Unit price before discount
					$up_excl_tax = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxunit - $this->posxqty - 0.8, 3, $up_excl_tax, 0, 'R', 0);
					...
					SetTwoDecimalsI(2);
					// Total HT line
					$total_excl_tax = 		getlinetotalexcltax_modI($object, $i, $outputlangs, $hidedetails);
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($total_ht + (!empty($object->remise) ? $object->remise : 0)), 0, $outputlangs,2,2,2), 0, 'R', 1);
		$pdf->SetXY(35, 288);
		$pdf->MultiCell(170, 4, '“Factura exenta de IVA por aplicación de la Directiva 2006/112/CE y el art. 25 de la Ley Ley 37/1992” ', 0, 'L');
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_ttc, 0, $outputlangs,2,2,2), $useborder, 'R', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetAlpha(0.5);//image/imagenes
		$pdf->Image(DOL_DOCUMENT_ROOT."/../documents/ecm/imagenes/fondo-factura.png", $this->marge_gauche, 80, 185, 0); // width=0 (auto)
		$pdf->SetAlpha(1);
		// Logo
...
function SetTwoDecimalsI($T)
{
	global $conf;
	
	
	if($conf->global->MAIN_MAX_DECIMALS_TOT)
	{
		$conf->global->MAIN_MAX_DECIMALS_TOT = $T;
	}
}
 ...
 */
function getlinetotalexcltax_modI($object, $i, $outputlangs, $hidedetails = 0)
{
	...
	$result = '';
		...
		elseif (empty($hidedetails) || $hidedetails > 1)
		{
			...
			if (!empty($object->lines[$i]->situation_percent) && $object->lines[$i]->situation_percent > 0)
			{
				...
				$result .= price($sign * ($total_ht / ($object->lines[$i]->situation_percent / 100)) * $progress, 0, $outputlangs,2,2,2);
			} else {
				$result .= price($sign * $total_ht, 0, $outputlangs,2,2,2);
			}
		}
