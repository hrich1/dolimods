...
/**
 * \file    test/phpunit/SinPrecioFunctionalTest.php
 * \ingroup sinprecio
 ...
 *
 * @package Testsinprecio
 */
class SinPrecioFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSinPrecio')->click();
		} else {
			// Disable the module
			$this->byHref('modSinPrecio')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/sinprecio/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('sinprecio/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/sinprecio/admin/about.php');
		$this->authenticate();
		return $this->assertContains('sinprecio/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('sinpreciowidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSinPrecio_SinPrecioTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSinPrecio_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
